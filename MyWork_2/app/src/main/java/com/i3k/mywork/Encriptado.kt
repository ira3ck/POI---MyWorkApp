    ...
    private val CIPHER_TRANSFORM = "AES/CBC/PKCS5PADDING"
     ...
     fun cifrar(textoPlano: String, llave: String) : String{
        ...
        val  cipher = Cipher.getInstance(CIPHER_TRANSFORM)
         ...
         System.arraycopy(llaveBytesOriginal,0,llaveBytesFinal,0,Math.min(llaveBytesOriginal.size,llaveBytesFinal.size))

        val secretKeySpec: SecretKeySpec = SecretKeySpec(llaveBytesFinal, "AES")

        val vectorInicializacion = IvParameterSpec(llaveBytesFinal)

        cipher.init(Cipher.ENCRYPT_MODE, secretKeySpec, vectorInicializacion)
     ...
     fun descifrar( textoCifrado: String, llave: String) : String{
        ...
        cipher.init(Cipher.DECRYPT_MODE, secretKeySpec, vectorInicializacion)
